...
export function checkHardcodedEntry(pool, config) {
  ...
  // ── CONDITIONAL BYPASS: High volatility + strong narrative + smart wallets ──
  if (
    pool.volatility >= 5 && 
    (pool.narrative_score >= 7 || pool.has_strong_narrative) && 
    (pool.smart_wallets_count >= 5 || pool.gmgn_smart_wallets >= 5)
  ) {
    // Relax minBinStep to 80 for high volatility pools with strong fundamentals
    if (pool.bin_step < 100 && pool.bin_step >= 80) {
      log("bypass", `BYPASS bin_step check for ${pool.name}: high vol (${pool.volatility}) + strong narrative + smart wallets`);
      return { pass: true, reason: "conditional bypass" };
    }
  }
  ...
  if (!Number.isFinite(feeActiveTvlRatio)) {
    const result = {
      pass: false,
      reason: "fee/TVL unknown (null/NaN) — hard reject until data valid",
    };
  ...
  // ── RULE 0b: Max Fee/TVL Cap (Trap Filter + Timeframe Scaling) ──
  // 649 trade audit: BigLoss average fee/TVL = 67.0% (vs Win avg 22.8%).
  // Anything above 50% (24h baseline) is almost always a low-TVL pool where fee/TVL ratio is artificially inflated.
  // Timeframe scaling: 5m pools naturally have higher fee/TVL (compressed volume window). Scale cap up for short timeframes.
  // Formula: cap = baseCap * (1440 / timeframeMinutes). 5m → 10x cap, 24h → 1x cap.
  const timeframeMinutes = (() => {
    const tf = String(s.timeframe ?? "5m").toLowerCase();
    const match = tf.match(/^(\d+)(m|h|d)$/);
    if (!match) return 5;
    const [, num, unit] = match;
    return parseInt(num) * (unit === "h" ? 60 : unit === "d" ? 1440 : 1);
  })();
  const baseMaxFee = Number(s.maxFeeActiveTvlRatio ?? 0.50);
  const scaledMaxFee = baseMaxFee * (1440 / timeframeMinutes);
  const maxFeeActiveTvlRatio = isWatchlisted(pool) ? scaledMaxFee + 1.0 : scaledMaxFee;
  if (Number.isFinite(feeActiveTvlRatio) && feeActiveTvlRatio > maxFeeActiveTvlRatio) {
    const result = {
      pass: false,
      reason: `fee/TVL ${feeActiveTvlRatio.toFixed(2)}% > max ${maxFeeActiveTvlRatio.toFixed(2)}% (trap: artificial high fee/TVL on low TVL or wash trading, scaled for ${s.timeframe ?? "5m"} timeframe${isWatchlisted(pool) ? ", even with watchlist boost" : ""})`,
    };
    recordHardcodedReject(baseMint, result.reason, s);
    return result;
  }
  ...
  // ── RULE 0d: Pool Price Change Validator (OBSERVATIONAL ONLY) ──
  // Enabled by config: rule0dPriceChange.enabled (default: false)
  // Docs in CLAUDE.md "P5 Price Change Telemetry"
  if (config.rule0dPriceChange?.enabled) {
    // future: filter on abs(pool_price_change_pct) thresholds
  }
