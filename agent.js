...
// For custom providers like gateway.dahono.com, uses fetch with proper Authorization header
let client;
...
// // Detect if it's a custom provider (not OpenAI/OpenRouter compatible)
// if (llmBaseUrl.includes('gateway.dahono.com')) {
...
// async function callCustomLLM(model, messages, tools, temperature, max_tokens) {
...
//       // Note: Custom providers may not support tools, so we omit them
//     })
//   });
//   
//   if (!response.ok) {
...
const DEFAULT_MODEL = "dahono/deepseek-v4-flash";
...
function isThinkingModeToolChoiceError(error) {
  const message = String(error?.message || error?.error?.message || error || "");
  return /tool_choice parameter does not support/i.test(message);
}
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        // Some thinking-mode providers return 400 with tool_choice=required instead of throwing.
        // Detect the response-level error and retry without tool_choice.
        const respErr = response?.error?.message || "";
        if (!omitToolChoice && /tool_choice.*not support|thinking mode.*tool_choice/i.test(respErr)) {
          omitToolChoice = true;
          log("agent", "Response-level thinking mode tool_choice error — retrying without tool_choice");
          attempt -= 1;
          continue;
        }
      ...
      if (!msg.tool_calls || (Array.isArray(msg.tool_calls) && msg.tool_calls.length === 0) || (!Array.isArray(msg.tool_calls) && Object.keys(msg.tool_calls).length === 0)) {
