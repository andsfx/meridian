...
export async function closePosition({ position_address, reason }) {
  ...
  try {
    ...
    // P2-RecoveryRecord: position may already be closed on-chain by concurrent action.
    // If so, still call recordPerformance to keep pool memory accurate.
    try {
      if (tracked && tracked.pool) {
        const poolAddr = tracked.pool;
        const deployedAt = new Date(tracked.deployed_at).getTime();
        const minsHeld = Math.floor((Date.now() - deployedAt) / 60000);
        const cached = _positionsCache?.positions?.find(p => p.position === position_address);
        const feesUsd = tracked.total_fees_claimed_usd || 0;
        const pnlPct = cached?.pnl_pct ?? tracked.peak_pnl_pct ?? 0;
        const pnlUsd = cached?.pnl_usd ?? 0;
        const signalSnapshot = resolvePerformanceSignalSnapshot({ poolAddress: poolAddr, baseMint: tracked.base_mint || null, tracked });
        await recordPerformance({
          position: position_address,
          pool: poolAddr,
          pool_name: tracked.pool_name || poolAddr.slice(0, 8),
          ...
          fees_earned_usd: feesUsd,
          final_value_usd: cached?.total_value_usd ?? 0,
          initial_value_usd: tracked.initial_value_usd || 0,
          minutes_in_range: Math.max(0, minsHeld),
          minutes_held: minsHeld,
          close_reason: reason || "agent decision (recovery)",
          signal_snapshot: signalSnapshot,
          entry_mcap: tracked.entry_mcap ?? null,
          entry_tvl: tracked.entry_tvl ?? null,
          entry_volume: tracked.entry_volume ?? null,
        ...
        recordClose(position_address, reason || "agent decision (recovery)");
        log("close_recover", `Recovered performance record for auto-closed position ${position_address.slice(0, 8)}: pnlPct=${Number(pnlPct).toFixed(2)}% fees=${feesUsd}`);
      }
